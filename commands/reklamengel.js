...
module.exports.run = async (client, message, args) => {
  ...
  if (!sub || !['aç', 'kapat', 'durum', 'log'].includes(sub)) {
    return message.channel.send({
      embeds: [new EmbedBuilder().setColor('Orange').setTitle('ℹ️ Reklam Engel Komutu').setDescription('Kullanım:\n`g!reklam-engel aç`\n`g!reklam-engel kapat`\n`g!reklam-engel durum`\n`g!reklam-engel log <#kanal>`')]
    });
  }
  ...
  if (sub === 'aç') {
    ...
    return message.channel.send({
      embeds: [new EmbedBuilder().setColor('Green').setTitle('✅ Reklam Engel Açıldı').setDescription('Artık reklam içeren mesajlar silinecek.')]
    });
  }
  ...
  if (sub === 'kapat') {
    ...
    return message.channel.send({
      embeds: [new EmbedBuilder().setColor('Red').setTitle('❌ Reklam Engel Kapatıldı').setDescription('Reklam engelleme sistemi devre dışı bırakıldı.')]
    });
  }
  ...
  if (sub === 'log') {
    ...
    return message.channel.send({
      embeds: [new EmbedBuilder().setColor('Green').setTitle('✅ Log Kanalı Ayarlandı').setDescription(`Reklam logları artık <#${kanal.id}> kanalına gönderilecek.`)]
    });
  }
};
