...
        .setLabel('Hızlı Ban')
        // Düzeltme: Özel emojinin ID'si kullanılarak setEmoji eklendi.
        .setEmoji({ id: '<:ban23:1449794687153209424>' }) // <a:ban:1449794687153209424> olduğu varsayıldı.
        .setStyle(ButtonStyle.Primary);

    const modalBanButton = new ButtonBuilder()
        .setCustomId(modalBanId)
        .setLabel('Sebep İle Banla')
        .setEmoji('📝') // Unicode emoji eklendi
        .setStyle(ButtonStyle.Danger);

    const cancelButton = new ButtonBuilder()
        .setCustomId(cancelId)
        .setLabel('İptal Et')
        .setEmoji('✖️') // Unicode emoji eklendi
...
            new ButtonBuilder().setCustomId(unbanId).setLabel('Banı Kaldır').setEmoji('🔨').setStyle(ButtonStyle.Success), // Emoji eklendi
            new ButtonBuilder().setCustomId(copyId).setLabel('ID Kopyala').setEmoji('📋').setStyle(ButtonStyle.Secondary), // Emoji eklendi
            new ButtonBuilder().setCustomId(closeId).setLabel('Kapat').setEmoji('❌').setStyle(ButtonStyle.Secondary), // Emoji eklendi
        );
